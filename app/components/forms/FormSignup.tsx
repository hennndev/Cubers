...
const FormSignup = () => {
  ...
  const loginGoogle = async () => {
    //login by google mail akan otomatis verified email
    await signIn("google", {
  ...
  return (
    <section className='w-[550px] pt-10 pb-10'>
      ...
      {isError && <section className='flexx relative mt-4 -mb-2 text-white font-normal bg-destructive rounded-lg p-3'>
        <LuTriangleAlert className='text-xl mr-2' />
        {isError}
        <LuX className='absolute top-2 right-2 text-lg cursor-pointer' onClick={() => setIsError(null)} />
      </section>}
      {/* handle success message */}
      {isSuccess && (
        <section className='flex relative mt-4 -mb-2 text-white font-normal bg-green-600 rounded-lg p-3'>
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className='mt-5 flex flex-col'>
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className='flex flex-col space-y-1.5 mb-3 flex-1'>
                ...
                <FormMessage />
                <FormDescription className='text-gray-500'>username must be unique</FormDescription>
              </FormItem>
            )}
          />
          <FormField
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className='flex flex-col space-y-1.5 mb-3 flex-1'>
                <label htmlFor="passwordConfirm" className='text-sm'>Password Confirm <span className='text-destructive'>*</span></label>
                <section className='flexx space-x-3'>
                  <FormControl>
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className='flex flex-col space-y-1.5 mb-3 flex-1'>
                <label htmlFor="passwordConfirm" className='text-sm'>Status<span className='text-destructive'>*</span></label>
                <section className='flexx space-x-3'>
                  <FormControl>
