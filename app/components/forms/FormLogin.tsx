...
const FormLogin = ({ emailRemember }: PropsTypes) => {
  ...
  const onSubmit = async (values: zod.infer<typeof LoginSchema>) => {
    ...
    try {
      ...
      // Jika tidak ok/atau ada error maka akan di catch oleh error handler
      if (res && !res.ok) {
        throw res.error
      } else {
  ...
  useEffect(() => {
    //Jika ada cookie rememberMe, maka akan set default value checkbox dari cookie value
    if (emailRemember) {
  ...
  return (
    <section className='w-[400px]'>
      ...
      {isError && (
        <section className='flexx relative mt-4 -mb-2 text-white font-normal bg-destructive rounded-lg p-3'>
          <LuTriangleAlert className='text-lg mr-2' />
          {isError}
