...
const FriendsTableHeader = ({ data }: PropsTypes) => {
  ...
  const handleAddFriend = async (e: React.MouseEvent<HTMLButtonElement, MouseEvent>, friendId: string, username: string) => {
    e.stopPropagation()
    try {
      await addFriend(userId, friendId)
      toast.success(`${username} has added as friend`)
    } catch (error) {
    ...
    <section className='relative w-full h-auto'>
      ...
      {searchTerm && (
        <SelectedSearchUser
          ...
          handleAdd={(e, userId, username) => handleAddFriend(e, userId, username)} />
      )}
    </section>
  )
}
